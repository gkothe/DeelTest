...
async function getBestClients({ app, ...params }, callback) {
...
async function getBestProffesion({ app, ...params }, callback) {
...
async function postBalanceDeposit({ app, ...params }, callback) {
    ...
    try {
        ...
        if (obj.deposit_value > maxAmountToDesposit) {
            throw ({ code: 404, msg: "You can't deposit more than 25% of your total of jobs to pay. The total you have to pay is " + Util.dollarUSLocale.format(maxAmountToDesposit) });
        }
